...
import { actSetUsername, actSetTypeGame, actSetSocket, actGetUsers, actAddUser, actLogout } from '../actions/user'
import { actGetRooms, actCreateRoom, actJoinRoom, actLeaveRoom, actPlayGame } from '../actions/room'
...
describe('room reducer', () => {
  ...
  it('LEAVE_ROOM', () => {
    ...
    input = { username: 'tester', isPlaying: false, roomId: null, rooms: [{ roomId: 'someRoom', players: ['michel'] }, { roomId: 'test', type: 'Classic', players: ['john', 'newPlayer'], running: false }] }
    expectedOutput = { username: 'tester', isPlaying: false, roomId: null, rooms: [{ roomId: 'someRoom', players: ['michel'] }, { roomId: 'test', type: 'Classic', players: ['newPlayer'], running: false }] }
    expect(reducer(input, action)).to.deep.equal(expectedOutput)
  })

  it('PLAY_GAME', () => {
    action = actPlayGame('test')
    input = { username: 'tester', isPlaying: false, roomId: 'test', rooms: [{ roomId: 'someRoom', players: ['michel'] }, { roomId: 'test', type: 'Classic', players: ['tester', 'john', 'newPlayer'], running: false }] }
    expectedOutput = { username: 'tester', isPlaying: true, roomId: 'test', rooms: [{ roomId: 'someRoom', players: ['michel'] }, { roomId: 'test', type: 'Classic', players: ['tester', 'john', 'newPlayer'], running: true }] }
    expect(reducer(input, action)).to.deep.equal(expectedOutput)
  })
})