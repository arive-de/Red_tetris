...
const genRoomId = async () => {
  ...
  if (roomIds.length === 0) {
    debug('no room yet');
    return String.fromCharCode(Math.floor(Math.random() * 26) + 97, Math.floor(Math.random() * 26) + 97, Math.floor(Math.random() * 10) + 48, Math.floor(Math.random() * 10) + 48);
  }

  let roomId = roomIds[0];

  while (roomIds.indexOf(roomId) >= 0) {
    roomId = String.fromCharCode(Math.floor(Math.random() * 26) + 97, Math.floor(Math.random() * 26) + 97, Math.floor(Math.random() * 10) + 48, Math.floor(Math.random() * 10) + 48);
    debug(roomId);
  }
